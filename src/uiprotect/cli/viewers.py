...
ARG_VIEWER_ID_PUBLIC = typer.Argument(..., help="Viewer ID")


@app.command("list-public")
def list_public(ctx: typer.Context) -> None:
    """List viewers (Public API)."""
...
def show_public(ctx: typer.Context, viewer_id: str = ARG_VIEWER_ID_PUBLIC) -> None:
    """Show details for a specific viewer (Public API)."""
...
def set_name_public(
    ctx: typer.Context,
    viewer_id: str = ARG_VIEWER_ID_PUBLIC,
    name: str = typer.Argument(..., help="New name for the viewer"),
) -> None:
    """Rename a viewer (Public API)."""
...
def set_liveview_public(
    ...
    """Assign a liveview to a viewer (Public API)."""
